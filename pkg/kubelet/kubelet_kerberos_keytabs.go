...
func (kl *Kubelet) makeKeytabMount(pod *v1.Pod, services string, podKDCClusters map[string]bool, user string) (*kubecontainer.Mount, error) {
	if services == "" {
		return nil, nil
	}
	...
	if !exists {
		kl.recorder.Eventf(pod, v1.EventTypeNormal, "MAKE_KEYTABS_MOUNT_START", "POD %s", pod.Name)
		if err := kl.createKeytab(keytabFilePath, pod, services, podKDCClusters, user, true); err != nil {
			kl.recorder.Eventf(pod, v1.EventTypeNormal, "MAKE_KEYTABS_MOUNT_FAILED", "POD %s , err %v", pod.Name, err)
			return nil, err
		}
		kl.recorder.Eventf(pod, v1.EventTypeNormal, "MAKE_KEYTABS_MOUNT_END", "POD %s", pod.Name)
	}
...
// Return:
// - error, if failed
func (kl *Kubelet) createKeytab(dest string, pod *v1.Pod, services string, podKDCClusters map[string]bool, user string, force bool) error {
	defer lock.ExecTime(time.Now(), "createKeytab", pod.Name)
	var err error
...
func (kl *Kubelet) refreshKeytab(clusterName, services string, hostKeyVersions, podKeyVersions map[string]int, force bool) error {
	defer lock.ExecTime(time.Now(), "refreshKeytab", clusterName)
...
// Return:
// - error, if failed
func (kl *Kubelet) invokePodKeytabRefresh(pod *v1.Pod, trimKeytab bool) error {
	return kl.krbManager.InvokePodKeytabRefresh(pod, trimKeytab)
}
...
// Return:
// - error, if failed
func (kl *Kubelet) verifyAndFixKeytab(pod *v1.Pod, services, hostname string, podAllClusters map[string]bool, podDir, userName string, withFix bool) error {
	defer lock.ExecTime(time.Now(), "verifyAndFixKeytab", pod.Name)
