...
func (kl *Kubelet) makeCertMount(pod *v1.Pod, podKDCClusters map[string]bool, user string) (*kubecontainer.Mount, error) {
	...
	if !areCertsValid {
		kl.recorder.Eventf(pod, v1.EventTypeNormal, "MAKE_CERTS_MOUNT_START", "POD %s", pod.Name)
		if err := kl.createCerts(certsFilePath, pod, podKDCClusters, user); err != nil {
			kl.recorder.Eventf(pod, v1.EventTypeNormal, "MAKE_CERTS_MOUNT_FAILED", "POD %s , err %v", pod.Name, err)
			return nil, err
		}
		kl.recorder.Eventf(pod, v1.EventTypeNormal, "MAKE_CERTS_MOUNT_END", "POD %s", pod.Name)
	}
...
// Return:
// - error, if failed
func (kl *Kubelet) createCerts(dest string, pod *v1.Pod, podKDCClusters map[string]bool, user string) error {
	defer lock.ExecTime(time.Now(), "createCerts", pod.Name)
...
// Return:
// - error, if failed
func (kl *Kubelet) refreshCerts(clusterName, certsDir, user string, pod *v1.Pod) error {
	defer lock.ExecTime(time.Now(), "refreshCerts", clusterName)
...
// Return:
// - error, if failed
func (kl *Kubelet) copyCertsToPod(hostDir, certsDir, clusterName, user string, removeOriginal bool, pod *v1.Pod) error {
	if removeOriginal {
