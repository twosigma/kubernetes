...
var map_PodSpec = map[string]string{
	...
	"setHostnameAsFQDN":             "If true the pod's hostname will be configured as the pod's FQDN, rather than the leaf name (the default). In Linux containers, this means setting the FQDN in the hostname field of the kernel (the nodename field of struct utsname). In Windows containers, this means setting the COMPUTERNAME to FQDN. If a pod does not have FQDN, this has no effect. Default to false.",
}
